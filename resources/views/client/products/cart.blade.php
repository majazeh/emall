@php
    $quantity = 0;
    if(isset($item) && $item instanceof \App\Models\InvoiceItem ){
        $quantity = $item->quantity;
    }elseif(isset($product->cart->quantity)){
        $quantity = $product->cart->quantity;
    }
@endphp
<div data-xhr="cart-item-{{ $product->id }}">
    <div data-cart="{{ $product->id }}" data-quantity="{{ $quantity }}">
        @if ($quantity)
        <div class="flex justify-between items-center">
            ...
            <span class="text-sm font-bold mx-2" data-quantityLog>{{ $quantity }}</span>
